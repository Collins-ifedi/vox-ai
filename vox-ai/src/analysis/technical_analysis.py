...
class TechnicalAnalyzer:
    """
    Analyzes OHLCV data to generate a wide range of technical indicators using TA-Lib
    ...
    def generate_all_indicators(self) -> Optional[pd.DataFrame]:
        """
        Calculates all defined technical indicators using TA-Lib and adds them to the DataFrame.
        Indicators not in TA-Lib are calculated manually.
        ...
        df = self.df_with_indicators
        # For TA-Lib, ensure we have the correct dtypes
        ...
        try:
            ...
            df['SMA_50'] = talib.SMA(cl, timeperiod=50)
            df['SMA_200'] = talib.SMA(cl, timeperiod=200)
            df['EMA_12'] = talib.EMA(cl, timeperiod=12)
            df['EMA_26'] = talib.EMA(cl, timeperiod=26)
            ...
            df['DMP_14'] = talib.PLUS_DI(hi, lo, cl, timeperiod=14)
            df['DMN_14'] = talib.MINUS_DI(hi, lo, cl, timeperiod=14)
            aroondown, aroonup = talib.AROON(hi, lo, timeperiod=14)
            df['AROOND_14'] = aroondown
            df['AROONU_14'] = aroonup
            df['SAR_0.02_0.2'] = talib.SAR(hi, lo, acceleration=0.02, maximum=0.2)
            ...
            df['MOM_10'] = talib.MOM(cl, timeperiod=10)
            df['ROC_12'] = talib.ROC(cl, timeperiod=12)
            ...
            # Manual Keltner Channels
            kc_ema = talib.EMA(cl, timeperiod=20)
            kc_atr = talib.ATR(hi, lo, cl, timeperiod=10) # pandas-ta uses ATR 10 for KC
            df['KCBe_20_2'] = kc_ema
            df['KCUe_20_2'] = kc_ema + (kc_atr * 2)
            df['KCLe_20_2'] = kc_ema - (kc_atr * 2)
            df['KCP_20_10_2'] = (df['close'] - df['KCLe_20_2']) / (df['KCUe_20_2'] - df['KCLe_20_2'])
            # Manual Donchian Channels
            df['DCU_20'] = df['high'].rolling(20).max()
            df['DCL_20'] = df['low'].rolling(20).min()

            # --- Volume Indicators ---
            df['OBV'] = talib.OBV(cl, vo)
            df['MFI_14'] = talib.MFI(hi, lo, cl, vo, timeperiod=14)
            # Manual Chaikin Money Flow (CMF)
            mfm = ((cl - lo) - (hi - cl)) / (hi - lo)
            mfv = mfm * vo
            df['CMF_20'] = mfv.rolling(20).sum() / vo.rolling(20).sum()
            # Manual VWAP (resets daily)
            df['VWAP_D'] = df.groupby(df.index.date).apply(lambda x: (x['close'] * x['volume']).cumsum() / x['volume'].cumsum()).reset_index(level=0, drop=True)

            # --- Final Calculations ---
            self._add_golden_death_crosses()
            num_gen_cols = len(self.df_with_indicators.columns) - len(self.df_ohlcv_original.columns)
            logger.info(f"Successfully generated TA indicators using TA-Lib. Added {num_gen_cols} columns.")
            return self.df_with_indicators.copy()

        except Exception as e:
            logger.error(f"Error generating technical indicators with TA-Lib: {e}", exc_info=True)
...
def main_cli():
    ...
    print("--- Technical Analysis Module CLI Test (TA-Lib Version) ---")
